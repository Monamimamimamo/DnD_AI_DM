...
- Всегда возвращай полный JSON объект со всеми полями""";
    ...
    public static String getEndpointSelectionSystemPrompt() {
        return "Ты — эксперт по правилам D&D 5e и структуре SRD API.\n\n" +
            ...
            "КРИТИЧЕСКИ ВАЖНО:\n" +
            "- Используй ТОЛЬКО точные имена эндпоинтов из списка доступных эндпоинтов в пользовательском промпте\n" +
            "- Каждое имя эндпоинта должно ТОЧНО совпадать с одним из имен в списке доступных эндпоинтов\n\n" +
...
Доступные эндпоинты SRD API (используй ТОЛЬКО эти точные имена):
%s

КРИТИЧЕСКИ ВАЖНО - ПРАВИЛА ИСПОЛЬЗОВАНИЯ ЭНДПОИНТОВ:
1. Используй ТОЛЬКО точные имена эндпоинтов из списка выше
2. Каждое имя должно ТОЧНО совпадать с одним из имен в списке доступных эндпоинтов
...
Отвечай ТОЛЬКО валидным JSON (используй ТОЧНО те имена эндпоинтов, которые есть в списке выше):
{
    "requires_check": true/false, // Требует ли действие проверки навыка/характеристики?
    "required_endpoints": ["точное_имя_эндпоинта_из_списка", "другое_точное_имя"]
}""";
